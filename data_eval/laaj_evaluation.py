...
def save_progress(df, filename):
    """Save the current progress by appending or rewriting to a single CSV file"""
...
def add_score_column(df_source, df_target, col1, col2, new_col_name, results_file):
    """
    Process each row and add scores to the target dataframe with regular saves to a single file
    """
    total_rows = len(df_source)
    
    try:
        for idx, row in tqdm(df_source.iterrows(), total=len(df_source), desc="Processing"):
            if should_interrupt:
                print(f"Processing interrupted at row {idx}/{total_rows}")
                save_progress(df_target, results_file)
                return df_target
...
if __name__ == "__main__":
    try:
        results_file = "data/benchmarking_nl_to_rtl/all_models_results/2_12_25/complete_results.csv"
        final_scores_file = "data/benchmarking_nl_to_rtl/all_models_results/2_12_25/final_results_scores.csv"
        
        
        output_df = pd.DataFrame()
        df = pd.read_csv("data/benchmarking_nl_to_rtl/all_models_results/2_12_25/final_results.csv")
        #df = pd.read_csv("data/test/test_input.csv")
        
        for col in df.columns[2:]:
            print(f"\nProcessing column: {col}")
            output_df = add_score_column(df, output_df, "ground_truth", col, col, results_file)
